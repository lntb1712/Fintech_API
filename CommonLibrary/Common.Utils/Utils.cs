...
namespace Common.Utils
{
    public static class Utils
    {
        ...
        public static bool IsPhoneNumber(string phoneNumber)
        {
            ...
            return Regex.IsMatch(phoneNumber, @"^(\+\d{1,3}[- ]?)?\d{10}$");
        }
        
        
        public static List<DateTime> GetWeekdaysInRange(DateTime start, DateTime end)
        {
            // Create a list to store the dates from Monday to Friday
            List<DateTime> weekdaysInRange = new List<DateTime>();

            // Iterate through the days within the specified range
            for (DateTime currentDate = start; currentDate <= end; currentDate = currentDate.AddDays(1))
            {
                ...
                {
                    weekdaysInRange.Add(currentDate);
                }
            }
        ...
        public static List<(DateTime, DateTime)> DivideDateRange(DateTime startDate, int noOfSprints)
        {
            List<(DateTime, DateTime)> phases = new();

            for (int i = 0; i < noOfSprints; i++)
            {
                DateTime phaseStart = startDate.AddDays(i * 7);
                DateTime phaseEnd = phaseStart.AddDays(7);

                phases.Add((phaseStart, phaseEnd));
            }
        ...
        private static void MappingKeyValuePairToObject<T>(PropertyInfo property, T obj, JsonElement? value)
        {
            ...
            try
            {
                if (value.HasValue)
                {
                    // Deserialize từ JsonElement về kiểu propertyType
                    var val = System.Text.Json.JsonSerializer.Deserialize(value.Value.GetRawText(), propertyType);
            ...
            {
                // Có thể log lỗi hoặc throw nếu cần
                //TODO: log lai loi    
                Console.WriteLine(
                    $"[Mapping Error] Property: {property.Name}, Type: {propertyType.Name}, Error: {ex.Message}");
                // Bạn cũng có thể throw lại nếu muốn dừng
                // throw;
            }
            ...
            // if (value.HasValue)
            // {
            //     var val = System.Text.Json.JsonSerializer.Deserialize(value.Value, propertyType);
        ...
        public static MicrosoftAccessTokenPayload DecodeMicrosoftAccessToken(string accessToken)
        {
            var handler = new JwtSecurityTokenHandler();
            var token = handler.ReadJwtToken(accessToken);

            var payload = new MicrosoftAccessTokenPayload
            {
                Audience = token.Audiences.FirstOrDefault(),
                Issuer = token.Issuer,
                IssuedAt = DateTimeOffset.FromUnixTimeSeconds(long.Parse(token.Payload.Iat.ToString())).UtcDateTime,
                ExpiresAt = DateTimeOffset.FromUnixTimeSeconds(long.Parse(token.Payload.Exp.ToString())).UtcDateTime,
                ...
                Email = token.Claims.FirstOrDefault(c => c.Type == "upn" || c.Type == "unique_name")?.Value,
                ObjectId = token.Claims.FirstOrDefault(c => c.Type == "oid")?.Value,
                TenantId = token.Claims.FirstOrDefault(c => c.Type == "tid")?.Value,
                IpAddress = token.Claims.FirstOrDefault(c => c.Type == "ipaddr")?.Value
            };
        ...
        public static void WriteLogRequest(ILog log, string logType, string userId, string paramsOrBody, HttpRequest request, string? message = null)
        {
            ...
            log4net.ThreadContext.Properties["paramsOrBody"] = paramsOrBody;
            log4net.ThreadContext.Properties["level"] = logType.ToUpper();
